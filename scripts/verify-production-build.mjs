...
for (const token of forbidden) if (html.includes(token)) throw new Error(`Prototype/secret leaked into production build: ${token}`);

for (const file of ['supabase-config.js','supabase-bridge.js','supabase-persistence.js','supabase-storage.js','supabase-messages.js','production-hardening.js']) {
