 ...
 * Supabase Auth is authoritative for login. Database hydration is deliberately
 * non-blocking so one slow/blocked table cannot trap the user on "Signing in…".
 */
(function () {
  'use strict';
  ...
  async function json(query, label) {
    const result = await withTimeout(query, 9000, label || 'Supabase request');
  ...
  async function getSession() {
    const r = await withTimeout(client.auth.getSession(), 9000, 'Authentication service');
    return r?.data?.session || null;
  }

  async function getActiveProfile(userId) {
    const profile = await json(
      client.from('metabolic_profiles').select('*').eq('user_id', userId).maybeSingle(),
  ...
  async function hydrate() {
    ...
    /* Persist the authenticated identity BEFORE doing bulk hydration. This is
       the important fix: a slow table must never leave the login button stuck. */
    ...
    try {
      ...
      const programs = await json(
        client.from('metabolic_programs').select('*, metabolic_program_schedule(*)').order('name'),
        'Programmes'
      );
      ...
      const recipes = await json(
        client.from('metabolic_recipes').select('*').eq('status', 'ACTIVE').order('recipe_date', {ascending:false}),
        'Recipes'
      );
      ...
      db.clients = clients.map(c => {
        ...
        const assignment = assignments
          .filter(a => a.client_id === c.id && a.status === 'ACTIVE')
        ...
        return {
          ...
          notes:notes.filter(n => n.client_id === c.id).map(n => ({
            id:n.id, at:n.created_at, byName:n.author_id || 'Staff', text:n.content,
            visible:n.client_visible, authorId:n.author_id, noteType:n.note_type
          })),
          checkins:checkins.filter(k => k.client_id === c.id).map(k => ({
            id:k.id, date:k.checkin_date, at:k.created_at, self:k.source === 'client',
            byName:k.created_by || 'Client', createdById:k.created_by, notes:k.notes || '',
            values:Object.fromEntries((k.metabolic_checkin_values || []).map(v => [v.metric_code, Number(v.value_numeric ?? v.value_text)]))
          })),
          visits:reports.filter(r => r.client_id === c.id).map(r => ({
            id:r.id, date:r.report_date, source:r.source_name || r.title,
            sourceFileId:r.source_file_id, createdAt:r.created_at, createdBy:r.created_by,
            values:Object.fromEntries((r.metabolic_report_measurements || [])
              .filter(m => m.value_numeric != null)
              .map(m => [m.marker_code, Number(m.value_numeric)]))
          })),
          programId:assignment?.program_id || old.programId || null,
          programStart:assignment?.start_date || old.programStart || null,
          diet:diet ? {
            plan:{
              calories:diet.calories_target,
              protein:diet.protein_target_g,
              notes:diet.general_guidance,
              favour:(diet.foods_to_favour || []).join('\n'),
              limit:(diet.foods_to_limit || []).join('\n'),
              updatedAt:diet.updated_at,
              updatedByName:diet.created_by || null
            },
            chart:{
              times:old.diet?.chart?.times || {},
              cells:Object.fromEntries((diet.metabolic_diet_chart_entries || []).map(e => [
                `${['sun','mon','tue','wed','thu','fri','sat'][e.day_of_week]}.${e.meal_slot}`, e.content
              ])),
              updatedAt:diet.updated_at,
              updatedByName:diet.created_by || null
            },
      ...
      db.programs = programs.map(p => ({
        ...
        schedule:(p.metabolic_program_schedule || []).map(s => ({
          id:s.id,
          label:s.title,
          freq:frequencyKey(s),
          kind:s.item_type === 'CHECKIN' ? 'log' : 'report',
          markers:Array.isArray(s.config?.markers) ? s.config.markers : [],
          selfLog:s.item_type === 'CHECKIN',
          note:s.config?.note || ''
        }))
      }));
      ...
      db.recipes = recipes.map(r => ({
        id:r.id,
        date:r.recipe_date || String(r.created_at).slice(0,10),
        title:r.title,
        meal:r.meal,
        prepMins:r.preparation_minutes,
        servings:r.servings,
        calories:r.calories,
        protein:r.protein_g,
        ...
        sharedWith:shares.filter(s => s.recipe_id === r.id && s.client_id).map(s => ({
          id:s.id, clientId:s.client_id, at:s.shared_at, byName:s.shared_by
        })),
        deleteRequest:null,
        at:r.created_at,
        byName:r.created_by || ''
      ...
      /* Keep the authenticated local identity and the existing local DB.
         The application can still open and the console shows the exact error
         in DevTools instead of trapping the login screen. */
  ...
  function backendError(error) {
    ...
    if (/failed to fetch|network|timed out/i.test(message)) return 'Unable to reach Supabase. Please check the connection and try again.';
    return message;
  }
  ...
  async function backendSignIn(form) {
    ...
    try {
      ...
      /* Validate the profile only. Do NOT wait for all application tables here. */
      const profile = await getActiveProfile(session.user.id);
      const role = localRole(profile.role);
      backend.sessionUserId = session.user.id;
      backend.role = role;
      ...
      /* Reload immediately. The normal boot process hydrates the database in
         the background and cannot hold the login button hostage. */
  ...
  async function boot() {
    ...
    try {
      const session = await getSession();
      if (session?.user) {
        hydrate().catch(error => console.error('[LIVYA] Background hydration failed:', error));
      }
    } catch (error) {
